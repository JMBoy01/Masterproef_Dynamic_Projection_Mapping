...
def main():
    folder_path = "./Images/Projector Calibration/School/Raw"
    calibration_images = [f for f in os.listdir(folder_path) if f.endswith('.png')]
    calibration_img_index = 0
    ...
    _, circle_pattern_centers = makeCirclePatternImages(circle_pattern_size, img_size=circle_pattern_img_size, space_between=100, circle_size=30)
    ...
    while calibration_img_index < len(calibration_images):
        image_path = os.path.join(folder_path, calibration_images[calibration_img_index])
        img = cv.imread(image_path)

        circle_pattern_img_index = int(calibration_images[calibration_img_index].split("_p")[1].replace(".png", ""))
        ...
        if key == 121: # y
            print("Adding image '" + calibration_images[calibration_img_index] + "' to calibration data.")
            ...
            print("NOT using image '" + calibration_images[calibration_img_index] + "' for calibration.")
            ...
            print("Invalid key: press either 'y' or 'n' to decide what to do with the image.")
            continue

        calibration_img_index += 1


    # Close window when done showing images
    cv.destroyAllWindows()

    # Intrinsic calibration of projector
